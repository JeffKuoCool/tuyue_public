...
public class GPUImageFilterTools {
    ...
    public static GPUImageFilter createFilterForType(final Context context, final FilterType type) {
        switch (type) {
            ...
            case LOOKUP_AMATORKA:
                ...
                return amatorka;
            case GAUSSIAN_BLUR:
                return new GPUImageGaussianBlurFilter(2);
            case CROSSHATCH:
    ...
    public static GPUImageFilter createBlendFilter(Context context, Class<? extends GPUImageTwoInputFilter> filterClass) {
        try {
            ...
            filter.setBitmap(BitmapFactory.decodeResource(context.getResources(), R.mipmap.ic_camera));
            return filter;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public static GPUImageFilter createBlendFilter(Context context, Class<? extends GPUImageTwoInputFilter> filterClass, int drawableId) {
        try {
            GPUImageTwoInputFilter filter = filterClass.newInstance();
            filter.setBitmap(BitmapFactory.decodeResource(context.getResources(), drawableId));
            return filter;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public static class FilterAdjuster {
        ...
        public Adjuster<? extends GPUImageFilter> get(){
            return adjuster;
        }
        ...
        public FilterAdjuster(final GPUImageFilter filter) {
            if (filter instanceof GPUImageSharpenFilter) {
                ...
                adjuster = new ImageAlphaBlendAdjuster().filter(filter);
            }
            else {
        ...
        private class GaussianBlurAdjuster extends Adjuster<GPUImageGaussianBlurFilter> {
            @Override
            public void adjust(final int percentage) {
                float blurSize = range(percentage, 0.0f, 2.0f);
                getFilter().setBlurSize(blurSize);
            }
        }
...
//                getFilter().setMix(range(percentage, 0.0f, 1.0f));
                getFilter().setMix(range(percentage, 0.0f, 0.5f));
            }
        }
        ...
        private class ImageAlphaBlendAdjuster extends Adjuster<GPUImageAlphaBlendFilter> {
            @Override
            public void adjust(final int percentage) {
                getFilter().setMix(range(percentage, 0.0f, 0.5f));
            }
