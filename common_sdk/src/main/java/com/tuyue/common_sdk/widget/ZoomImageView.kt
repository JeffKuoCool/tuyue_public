 ...
 * 对图片进行放大缩小
 */
class ZoomImageView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : AppCompatImageView(context!!, attrs, defStyleAttr), ViewTreeObserver.OnGlobalLayoutListener {
    ...
    private lateinit var mAnimator //双击缩放动画
            : ValueAnimator
    ...
    private lateinit var translationAnimation //惯性移动动画
            : ValueAnimator
    ...
    private val doubleDrowScale: Float
        get() {
            ...
            var drowScale = 1.0f
            var scale = scale
            if (Math.abs(mInitScale - scale) < deviation) scale = mInitScale
            if (Math.abs(mMidScale - scale) < deviation) scale = mMidScale
            if (Math.abs(mMaxScale - scale) < deviation) scale = mMaxScale
            if (scale != mMidScale) {
                //当前大小不等于mMidScale,则调整到mMidScale
                drowScale = mMidScale
